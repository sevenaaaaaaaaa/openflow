...
// 置顶区：单独挑出
$pinnedEvents = array_values(array_filter($events, fn($e) => !empty($e['pinned'])));
$pinnedIds = array_map(fn($e) => $e['id'], $pinnedEvents);
...
.pin-badge{background:var(--accent-soft);color:var(--accent-strong);font-weight:700}
.ev-list.pinned{border-color:var(--accent);border-radius:var(--r-md);background:color-mix(in oklab,var(--accent),transparent 96%);padding:0 10px}
...
<main id="main" data-od-id="main">
  ...
  <section id="list" class="sec reveal" data-od-anchor data-od-id="events-list">
    ...
    <div class="sec-head row" style="margin-bottom:18px">
      <div><span class="kicker">置顶活动</span><h2>主办方推荐</h2></div>
    </div>
    <div class="ev-list pinned">
      <?php foreach ($pinnedEvents as $e): $sd = strtotime($e['start_date'] ?? ''); $ed = strtotime($e['end_date'] ?? ''); $past = $ed && $ed < $now; $online = ($e['event_type']??'')==='online'; ?>
      <a href="/events/<?=urlencode($e['slug'])?>" class="ev<?=$past?' past':''?>">
        <div class="ev-date"><b><?=$sd ? date('d', $sd) : '--'?></b><span><?=$sd ? date('M', $sd) : ''?></span></div>
        <div class="ev-body">
          <div class="row">
            <span class="badge pin-badge">📌 置顶</span>
    ...
    <?php endif; ?>
    <div class="ev-list">
      <?php foreach ($events as $e): if (!empty($e['pinned'])) continue; $sd = strtotime($e['start_date'] ?? ''); $ed = strtotime($e['end_date'] ?? ''); $past = $ed && $ed < $now; $online = ($e['event_type']??'')==='online'; ?>
      <a href="/events/<?=urlencode($e['slug'])?>" class="ev<?=$past?' past':''?>">
        <div class="ev-date"><b><?=$sd ? date('d', $sd) : '--'?></b><span><?=$sd ? date('M', $sd) : ''?></span></div>
        <div class="ev-body">
          <div class="row">
