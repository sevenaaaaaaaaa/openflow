...
> **v2.0** · 基于 TIPS，同时运行 Flow 与 Loop 的开源增长操作系统。
...
**形态**：一套 PHP 后端，多端触达 —— Web 后台 / **PWA 可安装**（mac·win·linux·iPad·安卓pad·鸿蒙 浏览器安装）/ **CLI**（`bin/of`）/ **MCP**（agent 可调 18+5 工具）。
...
**技术栈**：PHP 8 + JSON/SQLite，零构建链（无 Node/Composer 依赖），原生 JS + SVG（PWA/画布均为原生实现）。
