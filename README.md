...
>
> 每周四统合当周迭代更新 **[产品迭代动态](#产品迭代动态)**，并与首页、关于我们、知识库、一页纸、官方 PPT、引流直播课、H5 落地页等对外内容源对齐。
...
**形态**：一套 PHP 后端，多端触达 —— Web 后台 / **PWA 可安装**（mac·win·linux·iPad·安卓pad·鸿蒙 浏览器安装）/ **CLI**（`bin/of`）/ **MCP**（agent 可调 18 个工具）。
...
- **私域直播**：RTMP/OBS 推流 + YouTube 同步，横屏 / 竖屏 9:16 / 抖音式沉浸全屏三种形态；弹幕、点赞、慢速模式与违禁词风控、推品弹窗、直播间内下单支付闭环
- **创作台（Creator Studio）**：一个入口产出深度专栏（选题→大纲→成文→草稿）、口播视频脚本、品牌化幻灯片（5 套主题 + design.md 品牌约束，可导出 PDF / 公开分享链接）
- **产品发现 Loop**：每天自动从 GitHub 发现新产品，AI 结构化生成文章草稿进入审核队列，内容管线自己长
- **插件平台 v2**：钩子之外，插件可注册 API 路由、后台菜单与设置页、前端插槽与资源、定时任务（见 [插件开发指南](docs/PLUGIN-DEV.md)）
...
- **生成式封面**：统一设计框架 + 按内容抽象符号 + 可换肤重抽，编辑在后台一键生成新封面
- **Live2D 看板娘**：后台 AI 助理多套形象衣橱（御姐/猫娘/少年/中性），人格化气泡交互，右键换装
...
- **私域直播**（OBS/YouTube 推流、横竖屏与沉浸全屏形态、弹幕互动与风控、直播带货下单闭环）；
- 角色权限、同意管理、审计、2FA、备份恢复与多租户基础。

### AI 与增长大脑基础

- **创作台（Creator Studio）**：深度专栏 / 视频脚本 / 品牌幻灯片三条引导式产线，AI 结构化产出，草稿直达审核；
- **产品发现 Loop**：每日自动发现 GitHub 新产品并生成文章草稿，人工审核后发布；
...
- **GEO 全网采集**：RSS + 搜索 API 双通道（SerpAPI/Google CSE/Bing/百度/自定义），AI 结构化提炼话题；
- `Demo 陪跑`提供与生产数据硬隔离的完整增长场景：增长大脑与黄金 Loop 读取同一批 Demo 画像、行为证据和模拟成交真相，可一键装载或重置。
...
- **插件平台 v2**：跨内容、CDP、CRM、自动化、SEO、支付的 hooks，外加插件自定义 API 路由、后台菜单/设置页、前端插槽/资源与定时任务（[插件开发指南](docs/PLUGIN-DEV.md)）；
...
- **品牌系统**：流环 Logo（SVG 矢量 + 多尺寸 PNG）+ `design.md` 品牌契约，AI 生成物与页面共用同一套品牌约束；
- **设计系统**：统一 tokens / 构建器 / 契约测试，38 页前台页面共享一套视觉规范；
- **全形态适配**：直播与幻灯片针对 iPhone / 三星 / 华为 / 小米主流机型与折叠屏做了适配矩阵，低端设备自动降级（关模糊、限动画、降轮询）；
...
- v2.5 起有了首个 Agent 运行时：AI 在白名单工具内为目标做链式执行，安全动作自动进行、中高风险动作等待人工批准，运行过程落盘可审计；但它仍是单会话的计划执行，不做过夜自主循环，也不等于完整的 TIPS Agent；
...
- 用户级模式偏好与 Agent 运行时（白名单工具 + 风险审批门）已就位，持续扩展其工具面与可验证性；
...
> 每周四更新：统合当周产品迭代与重点修复，并回读首页、关于我们、知识库、一页纸、官方 PPT、引流直播课、H5 落地页、直播课幻灯片等对外内容源，确保本页描述与网站实际一致。完整变更明细见 [变更日志](md-docs/CHANGELOG.md)。

### 2026-09-07 ~ 09-13 · v2.5 冲刺周（122 commits）

**对外内容与信任资产**
- 定价页上线：开源免费 / 云托管两档 / 私有化议价；
- 前台叙事 v2.5 分三批落位：行动脊柱进入对外叙事，旧数字清零并对齐当前实际（首页 / 关于我们 / 一页纸 / GTM deck 同源更新）；
- 品牌：流环 Logo 全端统一（渐变锁定官方色 + PWA 图标重生成），全站弃用「自生长 AI Engine」旧措辞，JSON-LD/OG 同源更新；
- 信任资产：种子评价 seeder 上线，产品页客户评价 3→6 条。

**知识库与 H5 打通**
- 知识库由 `bin/seed-help.php` 自知识 9 篇冷启动；
- H5 落地页（`/lp/solo-growth.html`）接入 CDP 全局身份，落地页流量可被统一识别与归因。

**产品能力（节选）**
- 热点雷达：多源抓取 + AI 聚类热度 → 一键转 GEO 选题；
- 自定义报表（任意维度×指标×筛选 + 下钻保存）与对话式 BI（自然语言问数→出图→追问→下钻）；
- Agentic 底座：目标→工具链自主执行 + 重试自愈 + 风险审批门；一句话指挥（自然语言指令→可执行计划→审批执行）；
- 全平台发布框架（适配器注册表 + 真发连接器 + 通用 webhook + 队列重试）与插件生态扩展（权限运行时强制 + 区块/CLI/MCP 扩展面）；
- 直播互动升级：抽奖 + 秒杀库存（防超卖）+ 实时成交看板 + 回放切片 + 连麦管理；课程↔直播打通（直播课=一等课时）；
- CDP 分群即服务（`/api/segments.php` 按令牌拉取分群）+ 转化链路闭合（CAPI 调度 + purchase 回传 + 漏斗归因）。

**修复与质量**
- 后台全局双击防护 + 提交 busy 态（192 页即时生效）；
- SEO 三连修：404 自动分诊（301/410/待审）、筛选空页软 404 noindex+canonical、sitemap 死链清理；
- 安全：SVG 上传清洗补齐 foreignObject/data:URI/animate-href 绕过向量；
- CDN 缓存：PWA 图标改用 v2 文件名 + manifest 版本参数，绕开 7 天 immutable 边缘缓存。
