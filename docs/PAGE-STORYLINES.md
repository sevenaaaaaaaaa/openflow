...
**动手前先 grep 目标是否已存在**（本文件 §六·五 的血泪条款）：
新加 URL / 循环 / 段落前，先确认「是不是早就有、只是数据为空所以看不到」。
实例：`sitemap.php` 原本已有 `/topic/{slug}` 循环（`topics.json` 空 → 无输出），我直接加新块导致
**14 个专题 URL 重复输出**；正确做法是补齐 hub `/topics` + 提权，而不是新增循环。
另外 sitemap 的文章循环已加**防御去重**（内容侧出现 slug 碰撞时不重复输出 URL）。
