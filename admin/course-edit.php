...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $course['prerequisites'] = array_values(array_filter(array_map('trim', (array)($_POST['prerequisites'] ?? []))));
    $course['certificate'] = !empty($_POST['certificate']);
  ...
  <div class="main">
    ...
    <form method="post" id="courseForm">
      ...
      <div class="card">
        ...
        <div class="field-row">
          <div class="field"><label>类型</label><select name="type"><?php require_once __DIR__ . '/../lib/CourseSystem.php'; foreach (course_types() as $tk => $tv): ?><option value="<?=htmlspecialchars($tk)?>" <?=$course['type']===$tk?'selected':''?>><?=htmlspecialchars($tv)?></option><?php endforeach; ?></select></div>
        ...
        <div class="field-row">
          <div class="field"><label>先修课程 <span class="hint">· 需先学完这些课程（Ctrl/⌘ 多选）</span></label>
            <select name="prerequisites[]" multiple size="4" style="min-height:88px"><?php foreach ($courses as $pc): if (($pc['id'] ?? '') === $course['id']) continue; ?><option value="<?=htmlspecialchars($pc['id'])?>" <?=in_array((string)$pc['id'], array_map('strval', (array)($course['prerequisites'] ?? [])), true) ? 'selected' : ''?>><?=htmlspecialchars($pc['title'])?></option><?php endforeach; ?></select>
          </div>
          <div class="field"><label>结业证书</label>
            <label style="display:flex;align-items:center;gap:8px;margin-top:6px"><input type="checkbox" name="certificate" value="1" <?=!empty($course['certificate'])?'checked':''?> style="width:16px;height:16px"> 完课后自动颁发可验证证书（认证课默认开启）</label>
          </div>
        </div>
      </div>
...
function addLessonToChapter(chapterIdx) {
  ...
  row.innerHTML =
    ...
    '<div class="lesson-detail">' +
      ...
      '<div class="field-row">' +
        '<div class="field"><label>课时类型</label><select name="chapter_lesson_type[' + chapterIdx + '][]" onchange="toggleQuizQ(this)"><option value="video">视频</option><option value="text">图文</option><option value="quiz">测验</option><option value="download">资料下载</option><option value="live">直播课</option></select></div>' +
