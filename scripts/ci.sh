...
hdr "1/4 契约与单元测试（tests/*_test.php）"
P=0; F=0
...
hdr "2/4 页面故事线审计（本体序列 / 缺必需 / 收口顺序 / CTA）"
...
hdr "3/4 设计护栏（新增 class 必须已在 CSS 定义）"
...
if [ "$FAST" -eq 0 ] && python3 -c "import playwright" >/dev/null 2>&1; then
  hdr "4/4 多设备 + 弹窗审计（Playwright，真实测量）"
  ...
  hdr "4/4 多设备 + 弹窗审计（已跳过：--fast 或未安装 playwright）"
fi
