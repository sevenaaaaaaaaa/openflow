...
function include_member_submit($member): void {
    echo '<div class="card p-8"><h2 class="text-xl font-bold mb-2">投稿文章</h2><p class="text-sm text-gray-600 mb-6">提交后由管理员审核，审核通过后发布。</p>' .
        ...
        '<div class="field"><label>分类</label><select name="category" style="width:100%;padding:10px 12px;border:1.5px solid var(--border);border-radius:10px"><option value="insight">增长洞察</option><option value="leadership">内容与 SEO</option><option value="wellbeing">AI 运营</option><option value="industry">行业实践</option></select></div>' .
