/**
 * 角色化内容（数据）—— 由 assets/role-content.js 迁入并类型化。
 *
 * 只替换文字、不改变视觉结构。运行时契约保持：本文件作为入口会被打包回
 * assets/role-content.js，并继续在 window 上暴露 OF_ROLES / OF_ROLE_ORDER。
 */

export type RoleKey = 'beginner' | 'dev' | 'power' | 'enterprise';
...
export const ROLE_ORDER: readonly RoleKey[] = ['beginner', 'power', 'dev', 'enterprise'];

export const ROLES: Record<RoleKey, RoleContent> = {
  beginner: {
    key: 'beginner',
    label: '我是新手',
    emoji: '🌱',
...
/* 运行时契约：与迁移前一致，页面脚本仍从这里读 */
declare global {
  interface Window {
    OF_ROLES?: Record<string, RoleContent>;
    OF_ROLE_ORDER?: readonly string[];
  }
}
