...
import { createDialogRegistry } from '../lib/dialog.ts';
import { filterCommands, moveIndex, shouldScrollIntoView } from '../lib/palette.ts';
(function () {
  ...
  function mount() {
    ...
    function renderPalette(query) {
      ...
      palShown.forEach(function (it, i) {
        ...
        a.addEventListener('mouseenter', function () { setPalActive(i); });
    ...
    function setPalActive(i) {
      ...
      var dir = shouldScrollIntoView(i, 0, 1);   // 由浏览器滚动兜底：nearest 更省心
      var act = nodes[i];
    ...
    palInput.addEventListener('keydown', function (e) {
      ...
      if (e.key === 'ArrowDown') { e.preventDefault(); setPalActive(moveIndex(palActive, palShown.length, 1)); return; }
      if (e.key === 'ArrowUp') { e.preventDefault(); setPalActive(moveIndex(palActive, palShown.length, -1)); return; }
      if (e.key === 'Enter') {
    ...
    /* ── 通用弹窗行为层（DOM 适配层）：状态机在 src/lib/dialog.ts，有单测 ──
    ...
    (function () {
      ...
      var registry = createDialogRegistry({
        ...
        onBroadcast: function (kind, h) {
          if (kind !== 'open') return;
          try { document.dispatchEvent(new CustomEvent('of:dialog-open', { detail: { el: h.el } })); } catch (e) {}
        }
      });
      document.addEventListener('keydown', function (e) {
      ...
      document.addEventListener('click', function (e) {
        registry.handleClick({
          target: e.target,
          contains: function (node, t) { return !!(node && node.contains && node.contains(t)); }
        });
      });
      window.OFDialog = {
        lock: lock,
        ...
        register: function (el, name, opts) {
          ...
          registry.register({
            id: name, el: el, selfIsScrim: el.getAttribute('data-scrim') !== 'false',
            open: baseOpen, close: baseClose
          });
          el.__ofOpen = function () { registry.open(name); };
          el.__ofClose = function () { registry.close(name); };
        }
      };
      /* 注册三个既有弹窗：开关仍走原函数，行为统一由状态机负责 */
      ...
      var palEl = g('palette');
      palEl.setAttribute('data-scrim', 'false');   // 命令面板是浮层，不是全屏遮罩
